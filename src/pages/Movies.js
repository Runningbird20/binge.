...
function CatalogView({
  ...
  // Unlike fetchCatalogWindows this must NOT be reshuffled by orderBatch;
  ...
  const fetchCatalogSortedPage = useCallback(async (offset, { includeCount = false, includeFacets = false } = {}) => {
    ...
    const result = await fetchMovieCatalogSegment({
      offset,
      limit: PAGE_SIZE,
      search: searchTerm,
      genre: genreValues,
  ...
  useEffect(() => {
    ...
    const cacheKey = buildCatalogCacheKey('movies', { genreValues, searchTerm, sortMode, kidsSafe });
    ...
    async function loadCatalog() {
      ...
      try {
        const probe = await fetchMovieCatalogSegment({
          offset: 0,
      ...
      try {
        ...
        const orderedPool = orderBatch(pool);
        setTotal(pool.length);
        setItems(orderedPool);
        setUsingFallbackCatalog(true);
        setLoading(false);
        // Not cached — the bundled snapshot is a last resort when both the
        // real catalog and legacy API are unreachable; caching it would risk
        // serving stale placeholder data once the real backend recovers.
      } catch {
        if (cancelled || requestTokenRef.current !== requestToken) return;
        setItems([]);
        setTotal(0);
...
export default function Movies() {
  ...
  const handleQuickAdd = useCallback(async (item) => {
    ...
    setDetailMessage('');
    try {
      const saved = await addWatchlistItem({ mediaType: 'movie', mediaId: item.id, media: item });
  ...
  useEffect(() => {
    ...
    async function loadOpenItem() {
      try {
        const item = await fetchMovieById(openId);
  ...
  async function openItemDetails(item, { browseOnly = false } = {}) {
    ...
    try {
      const detailedItem = await fetchMovieById(item.id);
      if (detailedItem?.id) {
